    ...
    if ( $view === 'yes' ) {
        ...
        if ( $unseen_notification == 0 ) {
            if ( isset( $_COOKIE['wnbell_last_id'] ) ) {
                ...
                global $wpdb;
                // Get the post_date of the post with ID $last_id
                $last_seen_date = $wpdb->get_var( $wpdb->prepare( "SELECT post_date FROM {$wpdb->prefix}posts WHERE ID = %d", $last_id ) );
                ...
                $sql = $wpdb->prepare( $query, $last_seen_date );
