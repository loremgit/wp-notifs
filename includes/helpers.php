...
function wnbell_replace_placeholders(  $content, $post_id  ) {
    ...
    $to_replace = array(
        ...
        '{{human_date}}' => sprintf( _x( '%1$s ago', '%2$s = human-readable time difference', 'wp-notification-bell' ), human_time_diff( get_post_time( 'U', true, $post_id ), current_time( 'timestamp', true ) ) ),
        '{{time}}'       => get_date_from_gmt( get_the_date( 'H:i', $post_id ), 'H:i' ),
