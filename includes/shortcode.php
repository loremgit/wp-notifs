    ...
    if ( $view === 'yes' ) {
        ...
        if ( count( $data ) > 0 ) {
            ...
            foreach ( $data as $single_notification_id ) {
                ...
                // Apply filters to modify target (e.g., open in a new tab)
                $target = apply_filters( 'wnbell_link_target', '_self' );
                $output .= '<a href="' . $link;
                $output .= '" onclick="wnbell_ajax_seen(' . $post_id;
                $output .= ');"';
                $output .= ' style="' . $a_style . '" class="' . $link_class . '" target="' . esc_attr( $target ) . '">';
                $output .= $thumbnail;
                $i = 0;
            ...
            $no_notifs = ( isset( $options['no_notifs'] ) && $options['no_notifs'] != '' ? stripslashes( base64_decode( $options['no_notifs'] ) ) : '<div class="wnbell_empty_box" id="wnbell_empty_box_id">' . __( 'No new notifications', 'wp-notification-bell' ) . '<div>' );
        ...
        if ( $start_query ) {
            global $wpdb;
            // Get the post_date of the post with ID $user_last_seen
            $user_last_seen_date = $wpdb->get_var( $wpdb->prepare( "SELECT post_date FROM {$wpdb->prefix}posts WHERE ID = %d", $user_last_seen ) );
            ...
            $count = $wpdb->get_var( $wpdb->prepare( $count_query, $user_last_seen_date ) );
