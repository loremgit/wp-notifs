...
<?php 
 ...
 <select name="recipient_role">
   <?php 
    ...
    echo $recipients;
    ?>"/>
    <p class="description">Comma-separated list of usernames to target users, case-sensitive (optional)</p>
   </td>
    </tr>
    <tr style="vertical-align:bottom">
    <th scope="row"><?php 
...
function wnbell_add_notification_fields(  $notification_id, $notification, $update  ) {
    ...
    if ( wp_is_post_revision( $notification_id ) ) {
        return;
    }
    if ( wp_is_post_autosave( $notification_id ) ) {
        return;
    }
    // if new post
    if ( !$update ) {
    ...
    if ( $notification->post_type == 'wnbell_notifications' && ($notification->post_status == 'publish' || $notification->post_status == 'draft') ) {
        ...
        if ( $notification->post_status == 'publish' ) {
            ...
            if ( isset( $_POST['recipient_username'] ) && sanitize_text_field( $_POST['recipient_username'] ) !== "" ) {
                if ( $notification->post_type == 'wnbell_notifications' && $notification->post_status == 'publish' ) {
                    // Check if we've already incremented for this save action
                    $has_incremented = get_post_meta( $notification_id, 'wnbell_incremented_once', true );
                    if ( $has_incremented ) {
                        return;
                    }
                    if ( apply_filters( 'wnbell_increment_count_on_post_update', $increment, $notification_id ) ) {
                        ...
                        foreach ( $blogusers as $key => $user ) {
                            ...
                            if ( in_array( $user->user_login, $recipient_username ) ) {
                                ...
                                // Mark as incremented
                                update_post_meta( $notification_id, 'wnbell_incremented_once', true );
                            }
